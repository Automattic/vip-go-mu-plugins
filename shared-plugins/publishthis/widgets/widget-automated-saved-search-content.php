...
class Publishthis_Automated_Saved_Search_Content_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct ( 'publishthis_automated_saved_search_content_widget', 'PublishThis: Automated Saved Search Content', array ('classname' => 'automated-saved-search-content-widget', 'description' => 'Display automated saved search content from PublishThis.' ) );
	}
	
	/**
	 * @desc Display Saved Searches widget output
	 */
	...
	/**
	 * @desc Prepare Saved Searches widget settings for saving
	 */
	...
	/**
	 * @desc Init and display Saved Searches widget setup form
	 */
...
<p>
	<label for="<?php echo $this->get_field_id('bundle_id'); ?>">Saved Search:</label> 
	<select name="<?php echo $this->get_field_name('bundle_id'); ?>" id="<?php echo $this->get_field_id('bundle_id'); ?>">
		<option value="-1" <?php selected($instance['bundle_id'], '-1'); ?>>None</option>
		<?php foreach($saved_searches as $saved_search) : ?>
			<option value="<?php echo $saved_search->bundleId; ?>" <?php selected($saved_search->bundleId, $instance['bundle_id']); ?>><?php echo $saved_search->displayName; ?></option>
		<?php endforeach; ?>
	</select>
</p>
<p>

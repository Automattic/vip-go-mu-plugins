...
class Publishthis_Automated_Feed_Content_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 
							'publishthis_automated_feed_content_widget', 
							'PublishThis: Automated Feed Content', 
							array ( 'classname'   => 'automated-feed-content-widget', 
									'description' => 'Display automated news/video content from a PublishThis feed.' ) );
	}
	
	/**
	 * @desc Display Feeds widget output
	 */
	function widget($args, $instance) {
		...
		if ($content) {
			...
			$publishthis->load_template( 'widgets/automated-feed-content.php' );
	...
	/**
	 * @desc Prepare Feeds widget settings for saving
	 */
	...
	/**
	 * @desc Init and display Feeds widget setup form
	 */
...
<p>
	<label for="<?php echo $this->get_field_id('feed_id'); ?>">Published Feeds:</label>
	<select name="<?php echo $this->get_field_name('feed_id'); ?>" id="<?php echo $this->get_field_id('feed_id'); ?>">
		...
		<?php foreach($feeds as $feed) : ?>
			<option value="<?php echo $feed['feedId']?>" <?php selected($feed['feedId'], $instance['feed_id']); ?>><?php echo $feed['displayName']; ?></option>
		<?php endforeach; ?>
	</select>
</p>
<p>

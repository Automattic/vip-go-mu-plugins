...
Cypress.Commands.add('wpCli', (command, ignoreFailures) => {
	...
	cy.exec(`./bin/search/wp-env-cli tests-wordpress "wp --allow-root ${escapedCommand}"`, options).then(
		(result) => {
			cy.wrap(result);
		},
	);
...
Cypress.Commands.add('wpCliEval', (command) => {
	...
	cy.exec(
		`./bin/search/wp-env-cli tests-wordpress "wp --allow-root eval-file wp-content/mu-plugins/${fileName}"`,
...
Cypress.Commands.add('maybeEnableFeature', (featureName) => {
	cy.wpCli('vip-search list-features').then((wpCliResponse) => {
		if (!wpCliResponse.stdout.match(new RegExp(featureName, 'g'))) {
			cy.wpCli(`vip-search activate-feature ${featureName}`);
		}
	});
});
...
Cypress.Commands.add('maybeDisableFeature', (featureName) => {
	cy.wpCli('vip-search list-features').then((wpCliResponse) => {
		if (wpCliResponse.stdout.match(new RegExp(featureName, 'g'))) {
			cy.wpCli(`vip-search deactivate-feature ${featureName}`);
		}
	});
});
...
Cypress.Commands.add('getTotal', (totalNumber) => {
	...
	cy.get('#search-dev-tools-portal')
		.invoke('text')
		.should('match', new RegExp(`${totalNumber} result`, 'g'));

...
class Search_Test extends WP_UnitTestCase {
	...
	public function test__vip_search_sends_queries_to_previous_version_when_upgrade_is_in_progress_and_current_version_is_next() {
		Constant_Mocker::define( 'VIP_ELASTICSEARCH_ENDPOINTS', array(
		...
		self::$mock_global_functions->expects( $this->exactly( 3 ) )
			...
			->with( $this->callback( function ( $url ) {
				return in_array( $url, [
					...
					'https://es-endpoint:9235/vip-123-post-1/_bulk', // Mirror to the previous version
				] );
			} ) )
			->willReturn([
				'response' => [ 'code' => 200 ],

...
class VIP_SMTP {
	function init() {
		/**
		 * Is VIP's SMTP-based mailer enabled?
		 *
		 * By default, all VIP environments are automatically configured to send
		 * transactional emails (e.g. password reset, new comment) generated by
		 * WordPreess using VIP SMTP servers.
		 *
		 * For cases, where a third party provider needs to be used for all emails
		 * from the site, including both marketing and transaction emails, this
		 * filter can be use to disable loading of the VIP SMTP mailer so that you
		 * can initialize and wire up your own.
		 *
		 * @param bool $is_enabled Is VIP SMTP enabled?
		 *
		 */
		$is_enabled = apply_filters( 'vip_smtp_enabled', true );
		if ( false === $is_enabled ) {
			return;
		}

 ...
 *
 * WPGraphQL registers an unauthenticated `sendPasswordResetEmail` mutation
 * (`wp-graphql/src/Mutation/SendPasswordResetEmail.php`) which generates a password reset
 * key and emails it. That bypasses the protections we apply to `wp-login.php` in
 * `security.php` (rate limiting and lockouts via `lostpassword_post`, ambiguous
 * confirmation messages, and restricted usernames), and can be used for user enumeration.
 *
 ...
 * `WPGraphQL` class) as its plugin file loads, so this is reliable from `plugins_loaded`
 * onwards.
 *
 * @return bool
 */
function vip_is_wp_graphql_active() {
 ...
 *
 * Runs on `plugins_loaded` and again on `init` to also cover sites that load WPGraphQL as
 * a Composer dependency of the theme rather than as a plugin. Both run well before
 * WPGraphQL builds its schema, which only happens while serving a GraphQL request.
 *
 * @return void
 */
function vip_maybe_disable_wp_graphql_password_reset_mutation() {
	if ( ! vip_is_wp_graphql_active() ) {
		return;
	}

	/**
	 * Filters whether the WPGraphQL `sendPasswordResetEmail` mutation is removed from the
	 * schema. Password resets should go through `wp-login.php`, which is rate limited and
...
add_action( 'plugins_loaded', 'vip_maybe_disable_wp_graphql_password_reset_mutation' );
add_action( 'init', 'vip_maybe_disable_wp_graphql_password_reset_mutation', 0 );
 ...
 * @return void
 */
function vip_remove_wp_graphql_password_reset_mutation() {
	// WPGraphQL >= 1.14: prevents the mutation and its Input/Payload types from being registered at all.
	if ( ! has_filter( 'graphql_excluded_mutations', 'vip_exclude_wp_graphql_password_reset_mutation' ) ) {
		add_filter( 'graphql_excluded_mutations', 'vip_exclude_wp_graphql_password_reset_mutation' );
	}

	// Fallback for WPGraphQL < 1.14, which has no exclusion list: drop the field from the RootMutation type.
	if ( ! has_filter( 'graphql_RootMutation_fields', 'vip_remove_wp_graphql_password_reset_field' ) ) {
		add_filter( 'graphql_RootMutation_fields', 'vip_remove_wp_graphql_password_reset_field' );
	}
}
 ...
 * @return mixed
 */
function vip_exclude_wp_graphql_password_reset_mutation( $excluded_mutations ) {
	if ( ! is_array( $excluded_mutations ) ) {
		return $excluded_mutations;
	}

	$excluded_mutations[] = 'sendPasswordResetEmail';
 ...
 *
 * The field is registered as `lcfirst( $mutation_name )`, so match case-insensitively.
 *
 * @param mixed $fields Fields registered to the RootMutation type, keyed by field name.
 * @return mixed
 */
function vip_remove_wp_graphql_password_reset_field( $fields ) {
	if ( ! is_array( $fields ) ) {
		return $fields;
	}

	foreach ( array_keys( $fields ) as $field_name ) {
		if ( is_string( $field_name ) && 0 === strcasecmp( $field_name, 'sendPasswordResetEmail' ) ) {
